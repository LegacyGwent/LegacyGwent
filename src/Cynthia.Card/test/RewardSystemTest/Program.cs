...
partial class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        if(!System.Net.IPAddress.IsLoopback(System.Net.IPAddress.Parse(lan)) && !System.Net.NetworkInformation.NetworkInterface.GetAllNetworkInterfaces().SelectMany(x=>x.GetIPProperties().UnicastAddresses).Any(x=>x.Address.ToString()==lan))
        ...
        try
        {
            ...
            await Group("legacy deck persistence and authoritative appearance",PremiumLegacyDeckCases);
    ...
    {
        var start=new ProcessStartInfo(OperatingSystem.IsWindows() ? "powershell.exe" : "pwsh"){UseShellExecute=false,CreateNoWindow=true,RedirectStandardOutput=true,RedirectStandardError=true};
