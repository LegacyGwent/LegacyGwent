...
namespace Cynthia.Card
{
    ...
    {//
        ...
        public async Task HandleEvent(BeforeRoundStart @event)
        {
            ...
            await Card.Effect.Transform(CardId.AuberonConqueror, Card,isForce:true);
        }
        ...
        public async Task HandleEvent(AfterTurnStart @event)
        {
            ...
            var list = Game.PlayersDeck[PlayerIndex].Where(x => (
                    x.CardInfo().CardType == CardType.Unit && x.HasAnyCategorie(Categorie.WildHunt))).ToList();

            if (list.Count() != 0 && (Game.PlayersHandCard[PlayerIndex].Count() % 3 == 1 ))
            {
                var cards = list.First();
                await cards.Effect.Summon(Card.GetLocation() + 1, Card);
            }
            return;
