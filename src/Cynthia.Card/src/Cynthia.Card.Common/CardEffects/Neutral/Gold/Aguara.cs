...
namespace Cynthia.Card
{
    ...
    {//择二：使最弱的友军单位获得5点增益；使手牌中的1个随机单位获得5点增益；对最强的1个敌军单位造成5点伤害；魅惑1个战力不高于5点的敌军“精灵”单位。
        ...
        {
            //以下代码基于 本卡不会buff自己
            var cards = new (string title, string message)[] { ("超然之力", "Aguara_1_BoostLowest"), ("幻象", "Aguara_3_BoostHand"), ("狂热攻势", "Aguara_2_DamageHighest"), ("诱拐", "Aguara_4_CharmElf") };
        ...
        private async Task<int> buffhand2()
        {
            var cards = Game.PlayersHandCard[Card.PlayerIndex]
                .Where(x => x.Status.Type == CardType.Unit && x.CardInfo().CardUseInfo == CardUseInfo.MyRow)
                .Mess(RNG).ToList();
            if (cards.Count() == 0)
            {
                return 0;
