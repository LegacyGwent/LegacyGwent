...
namespace Cynthia.Card
{
    ...
    {//部署：从牌库召唤1张同名牌至同排。每当有敌方单位转为受伤状态时，获得1点增益。
        ...
        {
            if (@event.Target.PlayerIndex != PlayerIndex && Card.Status.CardRow.IsOnPlace() && @event.Target.Status.CardRow == Card.Status.CardRow && !Card.IsDead)
            {
