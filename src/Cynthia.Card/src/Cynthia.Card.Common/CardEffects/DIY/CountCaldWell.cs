...
namespace Cynthia.Card
{
    ...
    {//择一：从牌库中打出一张战力不高于自身的铜色单位，在回合结束把它送进墓地；或吞噬牌库中一张战力高于自身的铜色单位牌，将它的战力作为自身的增益。
        ...
        public override async Task<int> CardUseEffect()
        {
            ...
            {
                Card.CardPoint();
                var list = Game.PlayersDeck[Card.PlayerIndex].Where(x => (x.CardPoint() > Card.CardPoint()) && x.Is(Group.Copper)).ToList();
        ...
        public async Task HandleEvent(AfterTurnOver @event)
        {
            if (_needKill)
            {
                await _target.Effect.ToCemetery(CardBreakEffectType.ToCemetery);
                _needKill=false;
            }
        }
    }
