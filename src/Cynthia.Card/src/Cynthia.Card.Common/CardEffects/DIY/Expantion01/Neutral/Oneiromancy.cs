...
namespace Cynthia.Card
{
    ...
    {//择一：创造己方起始牌组中的1张非“密探”牌；或将4张随机非同名手牌变为“解梦术”。
        ...
        public override async Task<int> CardUseEffect()
        {
            ...
            if (switchCard == 0)
            {
                ...
                return await Card.CreateAndMoveStay(list.Where(x=>!x.HasAnyCategorie(Categorie.Agent)).Select(x => x.CardId).Mess(RNG).Take(4).ToArray());
            }
            if (switchCard == 1)
            {
                var cards = Game.PlayersHandCard[PlayerIndex].Where(x=>x.CardInfo().CardId!=Card.CardInfo().CardId).Mess(RNG).Take(4);
