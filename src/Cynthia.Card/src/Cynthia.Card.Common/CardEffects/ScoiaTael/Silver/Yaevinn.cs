...
namespace Cynthia.Card
{
	...
	{//间谍、力竭。 抽1张“特殊”牌和单位牌。保留1张，放回另一张。
		...
		public override async Task<int> CardPlayEffect(bool isSpying,bool isReveal)
		{
            ...
            var card1 = Game.PlayersDeck[AnotherPlayer].FilterCards(type: CardType.Special).Mess(Game.RNG).FirstOrDefault();
            var card2 = Game.PlayersDeck[AnotherPlayer].FilterCards(type: CardType.Unit).Mess(Game.RNG).FirstOrDefault();

            var list = new List<GameCard> {card1, card2};
