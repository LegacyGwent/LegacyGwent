...
namespace Cynthia.Card.Server
{
    public class GwentMatchs
    {
        ...
        public GwentMatchs(
            Func<IHubContext<GwentHub>> hub,
            GwentCardDataService gwentCardTypeService,
            GwentServerService gwentService
        )
        {
            _hub = hub;
        ...
        public async void StartGame(GwentRoom room, bool isSpecial = false, bool isCountMMR = false)
        {
            ...
            {
                await _hub().Clients
                    .Client((room.Player1 as ClientPlayer).CurrentUser.ConnectionId)
                    .SendAsync("MatchResult", true);
            }
            if (room.Player2 is ClientPlayer)
            {
                await _hub().Clients
                    .Client((room.Player2 as ClientPlayer).CurrentUser.ConnectionId)
                    .SendAsync("MatchResult", true);
            }
            //初始化房间
            var player1 = room.Player1;
            var player2 = room.Player2;
            var gwentGame = new GwentServerGame(
                player1,
                player2,
                _gwentCardTypeServic,
                result =>
                    _gwentService.InvokeGameOver(
                        result,
                        (player1 is AIPlayer || player2 is AIPlayer),
                        isCountMMR
                    ),
                isSpecial
            );
            ...
            if (room.Player1 is ClientPlayer)
            {
                (room.Player1 as ClientPlayer).CurrentUser.UserState =
                    room.Player2 is AIPlayer ? UserState.PlayWithAI : UserState.Play;
                (room.Player1 as ClientPlayer).CurrentUser.LastOpponentId = room.Player2 is AIPlayer ?null:(room.Player2 as ClientPlayer).CurrentUser.ConnectionId;
            }
            if (room.Player2 is ClientPlayer)
            {
                (room.Player2 as ClientPlayer).CurrentUser.UserState =
                    room.Player1 is AIPlayer ? UserState.PlayWithAI : UserState.Play;
                (room.Player2 as ClientPlayer).CurrentUser.LastOpponentId = room.Player1 is AIPlayer ?null:(room.Player1 as ClientPlayer).CurrentUser.ConnectionId;
            }
            //开启游戏
        ...
        public void PlayerJoin(ClientPlayer player, string password)
        {
            ...
            foreach (var room in GwentRooms)
            {
                ...
                if (
                    !room.IsReady
                    && (
                        (room.Password.ToLower() == password.ToLower() && !room.InBlacklist(player))
                        || (room.Password == string.Empty && password.ToLower().StartsWith("ai"))
                    )
                )
                {
        ...
        { //停止匹配,如果玩家没有正在匹配,返回false
            foreach (var room in GwentRooms)
            {
                ...
                if (
                    !room.IsReady
                    && room.Player1 != null
                    && room.Player1 is ClientPlayer
                    && (room.Player1 as ClientPlayer).CurrentUser.ConnectionId == ConnectionId
                )
                {
                    //将这个玩家的状态设置为 "闲置"
                    (room.Player1 as ClientPlayer)
                        .CurrentUser
                        .UserState = UserState.Standby;
                    //删除掉这个房间
                    GwentRooms.Remove(room);
                    //发送匹配结果,false
                    await _hub().Clients
                        .Client((room.Player1 as ClientPlayer).CurrentUser.ConnectionId)
                        .SendAsync("MatchResult", false);
                    //将用户中的"当前玩家"设置为空
                    (room.Player1 as ClientPlayer)
                        .CurrentUser
                        .CurrentPlayer = null;
                    ...
                    !room.IsReady
                    && room.Player2 != null
                    && room.Player2 is ClientPlayer
                    && (room.Player2 as ClientPlayer).CurrentUser.ConnectionId == ConnectionId
                )
                {
        ...
        { //对局中离开, 如果玩家没有正在对局,返回false
            foreach (var room in GwentRooms)
            {
                ...
                )
                {
                    ...
                    _ = room.CurrentGame.GameEnd(
                        room.CurrentGame.Player2Index,
                        exception,
                        isSurrender
                    );
                    return true;
        ...
        public bool LeaveViewList(User user, string roomId)
        {
            if (roomId == "")
            {
                foreach (var room in GwentRooms)
                {
                    if (
                        room.IsReady
                        && room.CurrentGame.ViewList.Any(
                            x => x.CurrentUser.ConnectionId == user.ConnectionId
                        )
                    )
                    {
