...
namespace Cynthia.Card.Server
{
    public class GwentServerGame : IGwentServerGame
    {
        ...
        public IList<(int PlayerIndex, GameCard CardId)> HistoryList { get; set; } =
            new List<(int, GameCard)>();
        ...
        public IList<Operation<ServerOperationType>>[] PlayerToResendToViewerInfo { get; set; } =
            new IList<Operation<ServerOperationType>>[2]
            {
                new List<Operation<ServerOperationType>>(),
                new List<Operation<ServerOperationType>>()
            };
        public (int? PlayerIndex, int HPoint) WhoHeight
        {
            get
            {
                var player1Row1Point = PlayersPlace[Player1Index][0]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus);
                var player1Row2Point = PlayersPlace[Player1Index][1]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus);
                var player1Row3Point = PlayersPlace[Player1Index][2]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus);
                var player2Row1Point = PlayersPlace[Player2Index][0]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus);
                var player2Row2Point = PlayersPlace[Player2Index][1]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus);
                var player2Row3Point = PlayersPlace[Player2Index][2]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus);
        ...
        private async Task DecideRedCoin()
        {
            ...
            var task1 = GetSelectMenuCards(
                Player1Index,
                selectList,
                isCanOver: false,
                title: "请选择你认为后手价值的点数"
            );
            var task2 = GetSelectMenuCards(
                Player2Index,
                selectList,
                isCanOver: false,
                title: "请选择你认为后手价值的点数"
        ...
        public async Task GameEnd(int winPlayerIndex, Exception exception, bool isSurrender = false)
        {
            ...
            var result = new GameResult()
            {
                ...
                RedPlayerGameResultStatus =
                    redIndex == winPlayerIndex ? GameStatus.Win : GameStatus.Lose,
                RedScore = new int[]
                {
                    PlayersRoundResult[0][redIndex],
                    PlayersRoundResult[1][redIndex],
                    PlayersRoundResult[2][redIndex]
                },
                BlueScore = new int[]
                {
                    PlayersRoundResult[0][blueIndex],
                    PlayersRoundResult[1][blueIndex],
                    PlayersRoundResult[2][blueIndex]
                },
                ...
                BlueBlacklistCode =
                    Players[blueIndex].Blacklist?.ToDeckModel().CompressDeck() ?? "",
            };
            GameResultEvent(result);
            TempGameResult = result;

            await Task.WhenAll(
                SendGameResult(winPlayerIndex, GameStatus.Win),
                SendGameResult(AnotherPlayer(winPlayerIndex), GameStatus.Lose)
            );
        ...
        public async Task BigRoundEnd() //小局结束,进行收场
        {
            ...
            {
                GameRound =
                    AnotherPlayer(RedCoin[CurrentRoundCount]) == Player1Index
                        ? TwoPlayer.Player1
                        : TwoPlayer.Player2;
                PlayersWinCount[Player1Index]++;
                PlayersWinCount[Player2Index]++;
            ...
            //-+/*/展示点数
            await Task.WhenAll(
                Players[Player1Index].SendAsync(
                    ServerOperationType.BigRoundShowPoint,
                    new BigRoundInfomation()
                    {
                        ...
                        GameStatus =
                            player2PlacePoint == player1PlacePoint
                                ? GameStatus.Draw
                                : (
                                    player2PlacePoint > player1PlacePoint
                                        ? GameStatus.Lose
                ...
                Players[Player2Index].SendAsync(
                    ServerOperationType.BigRoundShowPoint,
                    new BigRoundInfomation()
                    {
            ...
            {
                await Task.WhenAll(
                    Players[Player1Index].SendAsync(ServerOperationType.BigRoundShowClose),
            ...
            await Task.WhenAll(
                Players[Player1Index].SendAsync(
                    ServerOperationType.BigRoundSetMessage,
                    RoundCount <= 1 ? "第 2 小局开始!" : "决胜局开始!"
                ),
                Players[Player2Index].SendAsync(
                    ServerOperationType.BigRoundSetMessage,
                    RoundCount <= 1 ? "第 2 小局开始!" : "决胜局开始!"
            ...
            await SendEvent(
                new AfterRoundOver(
                    ...
                    player1PlacePoint,
                    player2PlacePoint,
                    player1PlacePoint == player2PlacePoint
                        ? (int?)null
                        : (player1PlacePoint > player2PlacePoint ? Player1Index : Player2Index)
                )
            );
            //888888888888888888888888888888888888888888888888888888888888888888888888
        ...
        public async Task<bool> PlayerRound()
        {
            ...
            await Players[AnotherPlayer(playerIndex)].SendAsync(
                ServerOperationType.SetCoinInfo,
                false
            );
            ...
            if (IsPlayersPass[playerIndex])
            {
                ...
                PlayersHandCard[playerIndex].Count + (IsPlayersLeader[playerIndex] ? 1 : 0) == 0
            )
            { //如果没有手牌,强制pass
            ...
            //获取信息
            var roundInfo = (await ReceiveAsync(playerIndex)).Arguments.ToArray()[
                0
            ].ToType<RoundInfo>(); //接收玩家的选择,提取结果
        ...
        public async Task PlayerBigRound(int player1Mulligan = 0, int player2Mulligan = 0)
        {
            //双方调度指定次数
            await Task.WhenAll(
                MulliganCard(Player1Index, player1Mulligan),
                MulliganCard(Player2Index, player2Mulligan)
            );
            //双方轮流进行游戏
            //1.根据GamRound进行一次流程
            ...
            while (await PlayerRound())
            {
                ...
                //3.切换回合
                GameRound = (
                    (GameRound == TwoPlayer.Player1) ? TwoPlayer.Player2 : TwoPlayer.Player1
                );
            }
        ...
        { //获取卡牌,手牌或者领袖,将那个GameCard存起来
            var card =
                cardInfo.HandCardIndex == -1
                    ? PlayersLeader[playerIndex][0]
                    : PlayersHandCard[playerIndex][cardInfo.HandCardIndex];
        ...
        //玩家抽卡
        public async Task<IList<GameCard>> LogicDrawCard(
            int playerIndex,
            int count,
            Func<GameCard, bool> filter = null
        )
        {
            filter ??= (x => true); //或许应该播放抽卡动画和更新数值
            if (count > PlayersDeck[playerIndex].Where(filter).Count())
                count = PlayersDeck[playerIndex].Where(filter).Count();
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                (
                    await LogicCardMove(
                        PlayersDeck[playerIndex].First(filter),
                        PlayersHandCard[playerIndex],
                        0,
                        autoUpdateDeck: false
                    )
                ).To(list.Add);
            }
        ...
        //封装的抽卡
        public async Task<(List<GameCard>, List<GameCard>)> DrawCard(
            int player1Count,
            int player2Count,
            Func<GameCard, bool> filter = null
        )
        {
            ...
            filter ??= (x => true);
            //抽卡限制,不至于抽空卡组
            if (player1Count > PlayersDeck[Player1Index].Where(filter).Count())
                player1Count = PlayersDeck[Player1Index].Where(filter).Count();
            if (player2Count > PlayersDeck[Player2Index].Where(filter).Count())
                player2Count = PlayersDeck[Player2Index].Where(filter).Count();
            var player1Task = DrawCardAnimation(
                Player1Index,
                player1Count,
                Player2Index,
                player2Count,
            ...
            var player2Task = DrawCardAnimation(
                Player2Index,
                player2Count,
                Player1Index,
                player1Count,
        ...
        //只有一个玩家抽卡
        public async Task<List<GameCard>> PlayerDrawCard(
            int playerIndex,
            int count = 1,
            Func<GameCard, bool> filter = null
        )
        ...
        public async Task<List<GameCard>> DrawCardAnimation(
            int myPlayerIndex,
            int myPlayerCount,
            ...
            for (var i = 0; i < myPlayerCount; i++)
            {
                await SendCardMove(
                    ...
                    new MoveCardInfo()
                    {
                        ...
                        Target = new CardLocation()
                        {
                            RowPosition = RowPosition.MyStay,
                            CardIndex = 0
                        },
                ...
                await SendCardMove(
                    ...
                    new MoveCardInfo()
                    {
                        ...
                        Target = new CardLocation()
                        {
                            RowPosition = RowPosition.MyHand,
                            CardIndex = 0
                        },
                    }
                );
            ...
            for (var i = 0; i < enemyPlayerCount; i++)
            {
                await SendCardMove(
                    ...
                    new MoveCardInfo()
                    {
                        ...
                        Target = new CardLocation()
                        {
                            RowPosition = RowPosition.EnemyStay,
                            CardIndex = 0
                        },
        ...
        public async Task MulliganCard(int playerIndex, int count)
        {
            if (PlayersDeck[playerIndex].Count <= 0)
                return;
            var mulliganStartOperation = Operation.Create(
                ServerOperationType.MulliganStart,
                PlayersHandCard[playerIndex].Select(x => x.Status),
                count
            );
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                PlayerToResendToViewerInfo[playerIndex].Add(
                ...
                await LogicCardMove(
                    mulliganCard,
                    PlayersDeck[playerIndex],
                    PlayersDeck[playerIndex].Count,
                    autoUpdateDeck: false
                );
                ...
                //----------------------------------------------------------------------
                var mulliganDataOperation = Operation.Create(
                    ServerOperationType.MulliganData,
                    mulliganCardIndex,
                    card.Status
                );
            ...
            foreach (var card in blackCardPool)
            {
                lock (RNG)
                {
                    PlayersDeck[playerIndex].Insert(
                        RNG.Next(PlayersDeck[playerIndex].Count() + 1),
                        card
                    );
                }
            }
            ...
            PlayerToResendToViewerInfo[playerIndex].Add(
                Operation.Create(ServerOperationType.MulliganEnd)
            );
        ...
        //<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
        //几个从用户那里获得信息的途径
        public Task<IList<int>> GetSelectMenuCards(
            int playerIndex,
            IList<CardStatus> selectList,
            int selectCount = 1,
            bool isCanOver = false,
            string title = "选择一张卡牌"
        ) //返回点击列表卡牌的顺序
        {
            return GetSelectMenuCards(
                playerIndex,
        ...
        public async Task<IList<GameCard>> GetSelectMenuCards(
            int playerIndex,
            IList<GameCard> selectList,
            int selectCount = 1,
            string title = "选择一张卡牌",
            bool isEnemyBack = true,
            bool isCanOver = true
        ) //返回点击列表卡牌的顺序
        {
                ...
                await GetSelectMenuCards(
                    ...
                    new MenuSelectCardInfo()
                    {
                        SelectList = selectList
                            .Select(card =>
                            {
                                return (!isEnemyBack || card.PlayerIndex == playerIndex)
                                    ? card.Status
                                    : new CardStatus(card.Status.DeckFaction); // { DeckFaction = card.Status.DeckFaction, IsCardBack = true };
                            })
                            .ToList(),
                        SelectCount = selectCount,
                        IsCanOver = isCanOver,
        ...
        public async Task<IList<int>> GetSelectMenuCards(int playerIndex, MenuSelectCardInfo info)
        {
            ...
            PlayerToResendToViewerInfo[playerIndex].Add(
                Operation.Create(ServerOperationType.SelectMenuCards, info)
            );
            var selected = (await ReceiveAsync(playerIndex)).Arguments.ToArray()[0].ToType<
                IList<int>
            >();
            PlayerToResendToViewerInfo[playerIndex].Clear();
            return selected;
        ...
        public async Task<IList<CardLocation>> GetSelectPlaceCards(
            int playerIndex,
        ...
        ) //指示器向边缘扩展格数
        {
            ...
            return (await ReceiveAsync(playerIndex)).Arguments.ToArray()[0].ToType<
                IList<CardLocation>
            >();
        }

        public async Task<RowPosition> GetSelectRow(
            int playerIndex,
            GameCard card,
        ...
        public async Task<IList<GameCard>> GetSelectPlaceCards(
            GameCard card,
            int count = 1,
            bool isEnemySwitch = false,
            Func<GameCard, bool> filter = null,
            SelectModeType SelectMode = SelectModeType.AllRow,
            CardType selectType = CardType.Unit,
            int range = 0,
            bool isHasConceal = false
        )
        {
            //自定义规则, 是否过滤特殊卡, 过滤自身
            var canSelect = GetGameCardsPart(
                ...
                (
                    x =>
                        ...
                        && //如果没有免疫
                        (
                            selectType == CardType.Unit
                                ? (GwentMap.CardMap[x.Status.CardId].CardType == CardType.Unit)
                                : (
                                    selectType == CardType.Special
                                        ? (
                                            GwentMap.CardMap[x.Status.CardId].CardType
                                            == CardType.Special
                                        )
                                        : true
                                )
            ...
            if (GameCardsPartCount(canSelect) < count)
                count = GameCardsPartCount(canSelect);
            if (count <= 0)
                return new List<GameCard>();
            var target = await GetSelectPlaceCards(
                ...
                {
                    CanSelect = canSelect,
                    SelectCard = isEnemySwitch
                        ? GetCardLocation(card.PlayerIndex, card).Mirror()
                        : GetCardLocation(card.PlayerIndex, card),
                    SelectCount = count,
                    Range = range
                }
            );
        ...
        public async Task<GameCard> LogicCardMove(
            GameCard source,
            IList<GameCard> target,
            int targetIndex,
            bool autoUpdateCemetery = true,
            bool autoUpdateDeck = true
        )
        {
            var player1SoureRow = (
                source.PlayerIndex == Player1Index
                    ? source.Status.CardRow
                    : source.Status.CardRow.Mirror()
            );
            ...
            if (
                autoUpdateCemetery
                && (player1SoureRow.IsInCemetery() || player1TagetRow.IsInCemetery())
            )
            {
        ...
        public async Task GameOverExecute()
        {
            ...
            var result = new GameResult()
            {
                ...
                BlueWinCount = PlayersWinCount[blueIndex],
                RedPlayerGameResultStatus =
                    PlayersWinCount[redIndex] > PlayersWinCount[blueIndex]
                        ? GameStatus.Win
                        : (
                            PlayersWinCount[redIndex] == PlayersWinCount[blueIndex]
                                ? GameStatus.Draw
        ...
        public IList<GameCard> RowToList(
            int myPlayerIndex,
            RowPosition row,
            bool isHasDead = false,
            bool isHasConceal = false
        )
        {
        ...
        {
            var row = (
                playerIndex == card.PlayerIndex ? card.Status.CardRow : card.Status.CardRow.Mirror()
            );
            var list = RowToList(playerIndex, row);
            return new CardLocation() { RowPosition = row, CardIndex = list.IndexOf(card) };
        }

        public CardLocation GetCardLocation(GameCard card) =>
            GetCardLocation(card.PlayerIndex, card);

        public int AnotherPlayer(int playerIndex) =>
            playerIndex == Player1Index ? Player2Index : Player1Index;

        public int GetPlayersPoint(int playerIndex)
        {
            var allcard = GetAllCard(playerIndex);
            return allcard
                .Where(x => (x.PlayerIndex == playerIndex && x.Status.CardRow.IsOnPlace()))
                .SelectToHealth()
                .Sum(x => x.health);
        }
        ...
        public async Task ClientDelay(int millisecondsDelay, int? playerIndex = null)
        {
            if (playerIndex == null || playerIndex == Player1Index)
                await Players[Player1Index].SendAsync(
                    ServerOperationType.ClientDelay,
                    millisecondsDelay
                );
            if (playerIndex == null || playerIndex == Player2Index)
                await Players[Player2Index].SendAsync(
                    ServerOperationType.ClientDelay,
                    millisecondsDelay
                );
        ...
        public GameCardsPart GetGameCardsPart(
            int playerIndex,
            Func<GameCard, bool> filter,
            SelectModeType selectMode = SelectModeType.All
        )
            ...
            if (selectMode.IsHaveMy())
            {
                if (selectMode.IsHaveHand())
                    PlayersHandCard[playerIndex]
                        .Select((x, i) => (x, i))
                        .Where(x => filter(x.x))
                        .ForAll(item => cardsPart.MyHandCards.Add(item.i));
                if (selectMode.IsHaveRow())
                {
                    PlayersPlace[playerIndex][0]
                        .Select((x, i) => (x, i))
                        .Where(x => filter(x.x))
                        .ForAll(item => cardsPart.MyRow1Cards.Add(item.i));
                    PlayersPlace[playerIndex][1]
                        .Select((x, i) => (x, i))
                        .Where(x => filter(x.x))
                        .ForAll(item => cardsPart.MyRow2Cards.Add(item.i));
                    PlayersPlace[playerIndex][2]
                        .Select((x, i) => (x, i))
                        .Where(x => filter(x.x))
                        .ForAll(item => cardsPart.MyRow3Cards.Add(item.i));
                }
            }
            if (selectMode.IsHaveEnemy())
            {
                if (selectMode.IsHaveHand())
                    PlayersHandCard[AnotherPlayer(playerIndex)]
                        .Select((x, i) => (x, i))
                        .Where(x => filter(x.x))
                        .ForAll(item => cardsPart.EnemyHandCards.Add(item.i));
                if (selectMode.IsHaveRow())
                {
                    PlayersPlace[AnotherPlayer(playerIndex)][0]
                        .Select((x, i) => (x, i))
                        .Where(x => filter(x.x))
                        .ForAll(item => cardsPart.EnemyRow1Cards.Add(item.i));
                    PlayersPlace[AnotherPlayer(playerIndex)][1]
                        .Select((x, i) => (x, i))
                        .Where(x => filter(x.x))
                        .ForAll(item => cardsPart.EnemyRow2Cards.Add(item.i));
                    PlayersPlace[AnotherPlayer(playerIndex)][2]
                        .Select((x, i) => (x, i))
                        .Where(x => filter(x.x))
                        .ForAll(item => cardsPart.EnemyRow3Cards.Add(item.i));
                }
            }
            return cardsPart;
        }
        ...
        public IList<GameCard> GetAllCard(
            int playerIndex,
            bool isContainDead = false,
            bool isHasConceal = false
        )
        {
        ...
        {
            var task1 = Players[Player1Index].SendAsync(
                ServerOperationType.SetAllInfo,
                GetAllInfo(TwoPlayer.Player1)
            );
            var task2 = Players[Player2Index].SendAsync(
                ServerOperationType.SetAllInfo,
                GetAllInfo(TwoPlayer.Player2)
            );
            await Task.WhenAll(task1, task2);
            await SetDeckInfo();
            // await SendOperactionList();
        ...
        {
            await Players[playerIndex].SendAsync(
                ServerOperationType.SetMyDeck,
                PlayersDeck[playerIndex]
                    .Select(x => new CardStatus(x.Status.CardId))
                    .OrderBy(x => x.CardId)
        ...
        {
            var player1Task = Players[playerIndex].SendAsync(
                ServerOperationType.SetMyCemetery,
                PlayersCemetery[playerIndex].Select(x => x.Status)
            );
            var player2Task = Players[AnotherPlayer(playerIndex)].SendAsync(
                ServerOperationType.SetEnemyCemetery,
                PlayersCemetery[playerIndex].Select(x => x.Status)
            );
        ...
        {
            var player1Task = Players[Player1Index].SendAsync(
                ServerOperationType.SetGameInfo,
                GetGameInfo(TwoPlayer.Player1)
            );
            var player2Task = Players[Player2Index].SendAsync(
                ServerOperationType.SetGameInfo,
                GetGameInfo(TwoPlayer.Player2)
            );
            return Task.WhenAll(player1Task, player2Task);
        }
        ...
        {
            var player1Task = Players[Player1Index].SendAsync(
                ServerOperationType.SetCardsInfo,
                GetCardsInfo(TwoPlayer.Player1)
            );
            var player2Task = Players[Player2Index].SendAsync(
                ServerOperationType.SetCardsInfo,
                GetCardsInfo(TwoPlayer.Player2)
            );
            return Task.WhenAll(player1Task, player2Task);
        }
        ...
        {
            var player1Task = Players[Player1Index].SendAsync(
                ServerOperationType.SetPointInfo,
                GetPointInfo(TwoPlayer.Player1)
            );
            var player2Task = Players[Player2Index].SendAsync(
                ServerOperationType.SetPointInfo,
                GetPointInfo(TwoPlayer.Player2)
            );
            return Task.WhenAll(player1Task, player2Task);
        }
        ...
        {
            var player1Task = Players[Player1Index].SendAsync(
                ServerOperationType.SetCountInfo,
                GetCountInfo(TwoPlayer.Player1)
            );
            var player2Task = Players[Player2Index].SendAsync(
                ServerOperationType.SetCountInfo,
                GetCountInfo(TwoPlayer.Player2)
            );
            return Task.WhenAll(player1Task, player2Task);
        }
        ...
        {
            var player1Task = Players[Player1Index].SendAsync(
                ServerOperationType.SetPassInfo,
                GetPassInfo(TwoPlayer.Player1)
            );
            var player2Task = Players[Player2Index].SendAsync(
                ServerOperationType.SetPassInfo,
                GetPassInfo(TwoPlayer.Player2)
            );
            return Task.WhenAll(player1Task, player2Task);
        }
        ...
        {
            var player1Task = Players[Player1Index].SendAsync(
                ServerOperationType.SetMulliganInfo,
                GetMulliganInfo(TwoPlayer.Player1)
            );
            var player2Task = Players[Player2Index].SendAsync(
                ServerOperationType.SetMulliganInfo,
                GetMulliganInfo(TwoPlayer.Player2)
            );
            return Task.WhenAll(player1Task, player2Task);
        }
        ...
        {
            var player1Task = Players[Player1Index].SendAsync(
                ServerOperationType.SetWinCountInfo,
                GetWinCountInfo(TwoPlayer.Player1)
            );
            var player2Task = Players[Player2Index].SendAsync(
                ServerOperationType.SetWinCountInfo,
                GetWinCountInfo(TwoPlayer.Player2)
            );
            return Task.WhenAll(player1Task, player2Task);
        }
        ...
        {
            var player1Task = Players[Player1Index].SendAsync(
                ServerOperationType.SetNameInfo,
                GetNameInfo(TwoPlayer.Player1)
            );
            var player2Task = Players[Player2Index].SendAsync(
                ServerOperationType.SetNameInfo,
                GetNameInfo(TwoPlayer.Player2)
            );
            return Task.WhenAll(player1Task, player2Task);
        }
        ...
        public GameInfomation GetGameInfo(TwoPlayer player)
        {
            ...
            {
                MyRow1Point = PlayersPlace[myPlayerIndex][0]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus),
                MyRow2Point = PlayersPlace[myPlayerIndex][1]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus),
                MyRow3Point = PlayersPlace[myPlayerIndex][2]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus),
                EnemyRow1Point = PlayersPlace[enemyPlayerIndex][0]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus),
                EnemyRow2Point = PlayersPlace[enemyPlayerIndex][1]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus),
                EnemyRow3Point = PlayersPlace[enemyPlayerIndex][2]
                    .Where(x => !x.Status.Conceal)
                    .Select(x => x.Status)
                    .Sum(x => x.Strength + x.HealthStatus),
                ...
                MyHandCount =
                    PlayersHandCard[myPlayerIndex].Count()
                    + (IsPlayersLeader[myPlayerIndex] ? 1 : 0),
                EnemyHandCount =
                    PlayersHandCard[enemyPlayerIndex].Count()
                    + (IsPlayersLeader[enemyPlayerIndex] ? 1 : 0),
        ...
        public GameInfomation GetCardsInfo(TwoPlayer player)
        {
            ...
            return new GameInfomation()
            {
                ...
                EnemyHandCard = PlayersHandCard[enemyPlayerIndex]
                    .Select(x => x.Status)
                    .Select(x => x.IsReveal ? x : new CardStatus(PlayersFaction[enemyPlayerIndex])), // { IsCardBack = true, DeckFaction = PlayersFaction[enemyPlayerIndex] }),
                MyPlace = PlayersPlace[myPlayerIndex]
                    .Select(x => x.Select(c => c.Status))
                    .ToArray(),
                EnemyPlace = PlayersPlace[enemyPlayerIndex]
                    .Select(
                        x =>
                            x.Select(c => c.Status)
                                .Select(
                                    item =>
                                        item.Conceal
                                            ? new CardStatus(PlayersFaction[enemyPlayerIndex]) /* { IsCardBack = true, DeckFaction = PlayersFaction[enemyPlayerIndex] } */
                                            : item
                                )
                    )
                    .ToArray(),
        ...
        public GameInfomation GetAllInfo(TwoPlayer player)
        {
            ...
            var result = new GameInfomation()
            {
                ...
                EnemyHandCard = PlayersHandCard[enemyPlayerIndex]
                    .Select(x => x.Status)
                    .Select(
                        x => x.IsReveal ? x : new CardStatus(PlayersFaction[enemyPlayerIndex]) /* { IsCardBack = true, DeckFaction = PlayersFaction[enemyPlayerIndex] }*/
                    )
                    .ToList(),
                MyPlace = PlayersPlace[myPlayerIndex]
                    .Select(x => x.Select(c => c.Status).ToList())
                    .ToArray(),
                EnemyPlace = PlayersPlace[enemyPlayerIndex]
                    .Select(
                        x =>
        ...
        {
            return Task.WhenAll(
                Players[playerIndex].SendAsync(ServerOperationType.ShowWeatherApply, row, type),
                Players[AnotherPlayer(playerIndex)].SendAsync(
                    ServerOperationType.ShowWeatherApply,
                    row.Mirror(),
                    type
                )
            );
        ...
        public Task SendCardOn(int playerIndex, CardLocation location)
        {
            ...
            return Players[playerIndex].SendAsync(ServerOperationType.CardOn, location);
        }
        ...
        public Task SendCardDown(int playerIndex, CardLocation location)
        {
            if (!location.RowPosition.IsOnRow())
                return Task.CompletedTask;
            return Players[playerIndex].SendAsync(ServerOperationType.CardDown, location);
        }

        public Task SendCardMove(int playerIndex, MoveCardInfo info)
        {
            return Players[playerIndex].SendAsync(ServerOperationType.CardMove, info);
        }
        ...
        public async Task SendSetCard(int playerIndex, GameCard card) //更新某个玩家的一个卡牌
        {
            ...
            //如果处于敌方场地
            var isBack =
                (card.Status.CardRow.IsOnPlace() && card.Status.Conceal)
                || (
                    card.PlayerIndex != playerIndex
                    && card.Status.CardRow.IsOnStay()
                    && card.Status.IsConcealCard
                );
            if (card.PlayerIndex != playerIndex)
                isBack |= (card.Status.CardRow.IsInHand() && (!card.Status.IsReveal));
            await Players[playerIndex].SendAsync(
                ServerOperationType.SetCard,
                GetCardLocation(playerIndex, card),
                isBack ? new CardStatus(card.Status.DeckFaction) : card.Status
            );
        }

        public async Task ShowCardMove(
            CardLocation location,
            GameCard card,
            bool refresh = true,
            bool refreshPoint = false,
            bool isShowEnemyBack = false,
            bool autoUpdateCemetery = true,
            bool autoUpdateDeck = true
        )
        {
            var isFromHide = card.Status.CardRow.IsInBack();
            var isShowPlayerIndexBack =
                (!location.RowPosition.IsMyRow() && isShowEnemyBack) || card.Status.Conceal;
            var isShowAnotherPlayerBack =
                (location.RowPosition.IsMyRow() && isShowEnemyBack)
                || card.Status.Conceal
                || card.Status.IsConcealCard;
            await SendCardMove(
                card.PlayerIndex,
                new MoveCardInfo()
                {
                    ...
                    Target = location,
                    Card = isShowPlayerIndexBack
                        ? card.Status.CreateBackCard()
                        : (
                            refresh
                                ? (
                                    (
                                        card.TagetIsShowBack(
                                            location,
                                            card.PlayerIndex,
                                            card.PlayerIndex
                                        )
                                ...
                                : (isFromHide ? card.Status.CreateBackCard() : null)
                        )
                }
            );
            await SendCardMove(
                AnotherPlayer(card.PlayerIndex),
                new MoveCardInfo()
                {
                    Source = GetCardLocation(AnotherPlayer(card.PlayerIndex), card),
                    Target = new CardLocation()
                    {
                        RowPosition = location.RowPosition.Mirror(),
                        CardIndex = location.CardIndex
                    },
                    Card = isShowAnotherPlayerBack
                        ? card.Status.CreateBackCard()
                        : (
                            refresh
                                ? (
        ...
        public async Task ShowCardDown(GameCard card) //落下
        {
            if (!card.Status.CardRow.IsOnRow())
                return;
            var task1 = Players[card.PlayerIndex].SendAsync(
                ServerOperationType.CardDown,
                GetCardLocation(card.PlayerIndex, card)
            );
            var task2 = Players[AnotherPlayer(card.PlayerIndex)].SendAsync(
                ServerOperationType.CardDown,
        ...
        //
        public Task ShowCardNumberChange(
            GameCard card,
            int num,
        ...
        public Task SendCardNumberChange(
            int playerIndex,
            GameCard card,
            int num,
            NumberType type = NumberType.Normal
        ...
        {
            if (
                card.IsShowBack(playerIndex)
                && !(type == NumberType.Countdown && playerIndex == card.PlayerIndex)
                && !card.Status.CardRow.IsInCemetery()
                && !(card.Status.CardRow.IsInDeck() && card.PlayerIndex == playerIndex)
            )
                return Task.CompletedTask;
        ...
        public Task SendGameResult(int playerIndex, GameStatus coerceResult = GameStatus.None) //是否强制指定比赛结果
        {
            ...
            if (PlayersWinCount[myPlayerIndex] < PlayersWinCount[enemyPlayerIndex])
                result = 2;
            //---
            return Players[myPlayerIndex].SendAsync(
                ServerOperationType.GameEnd,
                new GameResultInfomation(
        ...
        public GwentServerGame(Player player1, Player player2, bool isSpecial = false)
            : this(player1, player2, new GwentCardDataService(), result => { }, isSpecial) { }

        public GwentServerGame(
            Player player1,
            Player player2,
            Action<GameResult> gameResultEvent,
            bool isSpecial = false
        ) : this(player1, player2, new GwentCardDataService(), gameResultEvent, isSpecial) { }

        public GwentServerGame(
            Player player1,
            Player player2,
            GwentCardDataService gwentCardTypeService,
            Action<GameResult> gameResultEvent,
            bool isSpecial = false
        )
        {
            ...
            //将卡组转化成实体,并且打乱牌组
            PlayersDeck[Player1Index] = player1.Deck.Deck
                .Select(
                    cardId =>
                        new GameCard(
                            this,
            ...
            PlayersDeck[Player2Index] = player2.Deck.Deck
                .Select(
                    cardId =>
                        new GameCard(
                            this,
        ...
        public CardLocation GetRandomCanPlayLocation(
            int playerIndex,
            bool isAtEnd = false,
            RowPosition onlyRow = RowPosition.None
        )
        {
            ...
            var a = new List<int>();
            if (PlayersPlace[playerIndex][0].Count < RowMaxCount && (notBlack || blackIndex == 0))
                a.Add(0);
            if (PlayersPlace[playerIndex][1].Count < RowMaxCount && (notBlack || blackIndex == 1))
                a.Add(1);
            if (PlayersPlace[playerIndex][2].Count < RowMaxCount && (notBlack || blackIndex == 2))
                a.Add(2);
            if (a.Count == 0)
                return null;
        ...
        //====================================================================================
        //卡牌事件处理与转发
        public async Task<GameCard> CreateCard(
            string cardId,
            int playerIndex,
            CardLocation position,
            Action<CardStatus> setting = null
        )
        {
        ...
        )
        {
            ...
            var creatCard = new GameCard(
                this,
                playerIndex,
                new CardStatus(cardId, PlayersFaction[playerIndex], RowPosition.None),
                cardId
            );
            ...
            {
                await Players[playerIndex].SendAsync(
                    ServerOperationType.CreateCard,
                    creatCard.Status,
                    creatCard.GetLocation()
                );
                await Players[AnotherPlayer(playerIndex)].SendAsync(
                    ServerOperationType.CreateCard,
                    (
                        (creatCard.IsShowBack(AnotherPlayer(playerIndex)))
                            ? creatCard.Status.CreateBackCard()
                            : creatCard.Status
                    ),
                    creatCard.GetLocation().Mirror()
                );
            }
            else
            {
                ...
                if (creatCard.Status.CardRow.IsOnPlace())
                {
                    await AddTask(async () =>
                    {
                        if (creatCard.Status.CardRow.IsOnPlace())
                        {
                            if (position.RowPosition.IsMyRow())
                            {
                                await creatCard.Effect.CardDown(
                                    false,
                                    false,
                                    true,
                                    (false, false),
                                ...
                                if (sendEvent)
                                {
                                    await creatCard.Effects.RaiseEvent(
                                        new CardDownEffect(false, false)
                                    );
                                }
                            }
                            else
        ...
        public async Task<int> CreateAndMoveStay(
            int playerIndex,
            string[] cards,
            int createCount = 1,
            bool isCanOver = false,
            string title = "选择生成一张卡"
        )
        {
        ...
        public bool JoinViewList(Viewer viewer)
        {
            if (ViewList.All(x => x.CurrentUser.UserName != viewer.CurrentUser.UserName))
            {
                ...
                var task = viewer
                    .SendAsync(ServerOperationType.SetAllInfo, GetAllInfo(TwoPlayer.Player1))
