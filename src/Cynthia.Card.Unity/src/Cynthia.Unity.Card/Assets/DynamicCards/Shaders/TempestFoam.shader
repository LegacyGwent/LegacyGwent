Shader "DynamicCards/TempestFoam" {
Properties {
...
_EdgeFeather("Mesh edge fade", Range(0.001,0.3))=0.06
...
float4 dc_pack_ISGN_1=float4(v1.x,v1.y,w1.x,w1.y);
  ...
  // Fade in the fixed mesh UV domain before the advected foam reaches its edge.
  // SheenQuad uses a diamond in UV space, not the full texture rectangle.
  float meshEdge = 0.5 - dot(abs(v1 - 0.5), float2(1.0, 1.0));
  o0.w *= smoothstep(0.0, _EdgeFeather, meshEdge);
