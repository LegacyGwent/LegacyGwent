Shader "DynamicCards/PortableCard"
{
    Properties
    {
        ...
        _MaxAlpha ("Maximum opacity", Float) = 1
        _AlphaPower ("Opacity exponent", Float) = 1
        _AlphaMultiplier ("Opacity multiplier", Float) = 1
        _AlphaFromLuma ("Density texture", Float) = 0
        _TintOnly ("Use density tint", Float) = 0
        _VertexColorStrength ("Vertex color", Range(0,1)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _NoiseAmount, _Reflection, _Brightness, _Cutoff, _VertexColorStrength;
            float _MaxAlpha,_AlphaPower,_AlphaMultiplier,_AlphaFromLuma,_TintOnly;
            ...
            v2f vert(appdata v)
            {
                ...
                o.color = lerp(fixed4(1,1,1,1), v.color, _VertexColorStrength) * _Color * _TintColor;
                return o;
            }
            fixed4 frag(v2f i):SV_Target
            {
                float2 noise = (tex2D(_NoiseTex, i.uv * _NoiseTex_ST.xy + _NoiseTex_ST.zw).rg - .5) * _NoiseAmount;
                fixed4 source = tex2D(_MainTex, i.uv + noise);
                fixed4 color = source * i.color;
                color.rgb = lerp(color.rgb,i.color.rgb,_TintOnly);
                color.a = min(_MaxAlpha,pow(saturate(lerp(source.a,1-source.r,_AlphaFromLuma)),max(.001,_AlphaPower))*_AlphaMultiplier*i.color.a);
