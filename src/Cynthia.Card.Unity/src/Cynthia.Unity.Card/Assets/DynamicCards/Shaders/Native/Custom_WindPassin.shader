Shader "DynamicCards/Native/Custom_WindPassin" {
Properties {
...
_Colorexponent("_Colorexponent",Float)=1.0
...
float4 dc_v_cb0[1];
dc_v_cb0[0]=float4(_Time.x,_Time.y,_Time.z,_Time.w);
...
dc_f_cb0[3]=float4(_TimeMultiplier,0,0,0);
dc_f_cb0[4]=float4(_TintColor.x,_TintColor.y,_TintColor.z,_TintColor.w);
dc_f_cb0[5]=float4(_OpacityMultiplier,0,0,0);
dc_f_cb0[6]=float4(_Mask_ST.x,_Mask_ST.y,_Mask_ST.z,_Mask_ST.w);
dc_f_cb0[7]=float4(_Alphaexponent,_Colormultiplier,_Colorexponent,0);
  ...
  r0.x = 1 + -r0.x;
  r0.x = v4.w * r0.x;
  r0.y = dc_f_cb1[0].y * dc_f_cb0[3].x;
  r1.y = -v4.x * r0.y + v1.y;
  r1.x = v1.x;
  r0.yz = r1.xy * dc_f_cb0[2].xy + dc_f_cb0[2].zw;
  r1.xyzw = _MainTexture.Sample(sampler_MainTexture, r0.yz).xyzw;
  r0.x = r1.w * r0.x;
  r0.x = log2(r0.x);
  r0.xy = dc_f_cb0[7].zx * r0.xx;
  r0.xy = exp2(r0.xy);
  r0.y = dc_f_cb0[5].x * r0.y;
  o0.xyz = r0.xxx * dc_f_cb0[7].yyy + dc_f_cb0[4].xyz;
  r0.xz = v1.xy * dc_f_cb0[6].xy + dc_f_cb0[6].zw;
  r1.xyzw = _Mask.Sample(sampler_Mask, r0.xz).xyzw;
  o0.w = r1.x * r0.y;
  return;
}
ENDCG
