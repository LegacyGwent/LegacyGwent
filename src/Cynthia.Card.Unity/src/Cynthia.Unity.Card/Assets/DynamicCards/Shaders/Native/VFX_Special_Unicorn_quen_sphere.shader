Shader "DynamicCards/Native/VFX_Special_Unicorn_quen_sphere" {
Properties {
...
_w("_w",Float)=0.6000000238418579
_speed_vertex("_speed_vertex",Float)=8.0
...
dc_v_cb0[7]=float4(_noise_vertex_ST.x,_noise_vertex_ST.y,_noise_vertex_ST.z,_noise_vertex_ST.w);
dc_v_cb0[8]=float4(0,0,0,0);
dc_v_cb0[9]=float4(0,0,0,0);
dc_v_cb0[10]=float4(_w,_speed_vertex,0,0);
  ...
  r0.x = dc_v_cb1[0].y * dc_v_cb0[10].y + v2.x;
  r0.y = v2.y;
  r0.xy = r0.xy * dc_v_cb0[7].xy + dc_v_cb0[7].zw;
  r0.xyzw = _noise_vertex.SampleLevel(sampler_noise_vertex, r0.xy, 0).xyzw;
  r0.xyz = r0.xyz * dc_v_cb0[10].xxx + v0.xyz;
...
dc_f_cb0[2]=float4(_colour_fresnel.x,_colour_fresnel.y,_colour_fresnel.z,_colour_fresnel.w);
dc_f_cb0[3]=float4(_colour_multiply,0,0,0);
dc_f_cb0[4]=float4(_colour_main.x,_colour_main.y,_colour_main.z,_colour_main.w);
dc_f_cb0[5]=float4(_fresnel_shelf,_cube_multiply,_cube_power,0);
dc_f_cb0[6]=float4(_colour_cube_map.x,_colour_cube_map.y,_colour_cube_map.z,_colour_cube_map.w);
dc_f_cb0[7]=float4(0,0,0,0);
dc_f_cb0[8]=float4(_texture1_ST.x,_texture1_ST.y,_texture1_ST.z,_texture1_ST.w);
dc_f_cb0[9]=float4(_texture2_ST.x,_texture2_ST.y,_texture2_ST.z,_texture2_ST.w);
  ...
  r0.xyzw = dc_f_cb1[0].yyyy * float4(0.0500000007,0.0500000007,-0.0399999991,-0.0399999991) + v1.xyxy;
  r0.xy = dc_f_cb0[8].xy * r0.xy;
  r0.zw = dc_f_cb0[9].xy * r0.zw;
  r0.zw = r0.zw * float2(2,2) + dc_f_cb0[9].zw;
  r1.xyzw = _texture2.Sample(sampler_texture2, r0.zw).xyzw;
  r0.xy = r0.xy * float2(1.5,1.5) + dc_f_cb0[8].zw;
  ...
  r2.xyz = dc_f_cb0[6].xyz * dc_f_cb0[5].yyy;
  r1.xyz = r2.xyz * r1.xyz;
  r1.xyz = log2(r1.xyz);
  r1.xyz = dc_f_cb0[5].zzz * r1.xyz;
  r1.xyz = exp2(r1.xyz);
  r0.xyz = r1.xyz + r0.xyz;
  r1.x = log2(r0.w);
  r0.w = rsqrt(r0.w);
  r0.w = 1 / r0.w;
  o0.w = dc_f_cb0[4].w * r0.w;
  r0.w = dc_f_cb0[5].x * r1.x;
  r0.w = exp2(r0.w);
  r1.xyz = dc_f_cb0[3].xxx * dc_f_cb0[2].xyz;
  r1.xyz = r1.xyz * r0.www;
  o0.xyz = r0.xyz * dc_f_cb0[4].xyz + r1.xyz;
  return;
}
ENDCG
}
