Shader "DynamicCards/Native/VFX_Common_Additive_ColorGradient" {
Properties {
...
dc_f_cb0[7]=float4(0,_FlipU,_MipLevel,0);
  ...
  r0.x = min(r1.w, r1.y);
  r0.x = r1.x + -r0.x;
  r0.x = r0.x * 6 + 1.00000001e-10;
  r0.z = r1.w + -r1.y;
  r0.x = r0.z / r0.x;
  r0.x = r1.z + r0.x;
  r0.xzw = float3(0,-0.333333343,0.333333343) + abs(r0.xxx);
  r0.xzw = frac(r0.xzw);
  r0.xzw = -r0.xzw * float3(2,2,2) + float3(1,1,1);
  r0.xzw = saturate(abs(r0.xzw) * float3(3,3,3) + float3(-1,-1,-1));
  r0.xzw = float3(-1,-1,-1) + r0.xzw;
  r1.x = r0.y * -2 + 1;
  r1.x = dc_f_cb0[6].x * r1.x + r0.y;
  r0.y = dc_f_cb0[5].z * r0.y;
  r1.x = r1.x / dc_f_cb0[5].x;
  r0.xzw = r1.xxx * r0.xzw + float3(1,1,1);
