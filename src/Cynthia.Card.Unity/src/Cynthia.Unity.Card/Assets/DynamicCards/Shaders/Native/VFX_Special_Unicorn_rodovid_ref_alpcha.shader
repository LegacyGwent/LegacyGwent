Shader "DynamicCards/Native/VFX_Special_Unicorn_rodovid_ref_alpcha" {
Properties {
...
dc_f_cb0[4]=float4(_refraction,0,0,0);
dc_f_cb0[5]=float4(_mask_ST.x,_mask_ST.y,_mask_ST.z,_mask_ST.w);
dc_f_cb0[6]=float4(_speed_1,0,0,0);
  ...
  r0.y = dc_f_cb1[0].y * dc_f_cb0[6].x + v1.y;
  r0.x = v1.x;
  ...
  r0.x = 1 + -r0.x;
  r0.z = sqrt(r0.x);
  r0.xy = dc_f_cb0[4].xx * r0.yz;
  r0.xy = v2.xy * r0.xy;
  r0.zw = v1.xy * dc_f_cb0[5].xy + dc_f_cb0[5].zw;
  r1.xyzw = _mask.Sample(sampler_mask, r0.zw).xyzw;
  r0.zw = v3.xy / v3.ww;
