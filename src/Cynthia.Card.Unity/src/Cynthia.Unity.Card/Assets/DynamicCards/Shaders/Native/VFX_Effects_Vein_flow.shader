Shader "DynamicCards/Native/VFX_Effects_Vein_flow" {
Properties {
...
dc_f_cb0[3]=float4(_VeinsTechnical_ST.x,_VeinsTechnical_ST.y,_VeinsTechnical_ST.z,_VeinsTechnical_ST.w);
dc_f_cb0[4]=float4(_VeinsNoise_ST.x,_VeinsNoise_ST.y,_VeinsNoise_ST.z,_VeinsNoise_ST.w);
dc_f_cb0[5]=float4(_Timemultiplier,0,0,0);
dc_f_cb0[6]=float4(_Addcolor.x,_Addcolor.y,_Addcolor.z,_Addcolor.w);
dc_f_cb0[7]=float4(_Veinglowmultiply,0,0,0);
...
float4 dc_pack_ISGN_1=float4(v1.x,v1.y,w1.x,w1.y);
  ...
  r0.w = cmp(r0.w < 0);
  if (r0.w != 0) discard;
  r1.y = dc_f_cb1[0].y * dc_f_cb0[5].x;
  r1.x = 0;
