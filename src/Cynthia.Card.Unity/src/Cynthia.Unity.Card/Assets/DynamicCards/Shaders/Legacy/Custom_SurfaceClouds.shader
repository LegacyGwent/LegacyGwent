Shader "DynamicCards/Legacy/Custom_SurfaceClouds" {
Properties {
_MainTex("_MainTex",2D)="white" {}
_MainTexDark("_MainTexDark",2D)="white" {}
_Noise("_Noise",2D)="white" {}
_USpeed("_USpeed",Float)=0.0
_VSpeed("_VSpeed",Float)=0.0
_NoiseBrightness("_NoiseBrightness",Float)=0.0
_NoiseContrast("_NoiseContrast",Float)=1.0
...
SamplerState sampler_MainTexDark;
SamplerState sampler_Noise;
Texture2D<float4> _MainTex;
Texture2D<float4> _MainTexDark;
...
dc_f_cb0[4]=float4(_MainTexDark_ST.x,_MainTexDark_ST.y,_MainTexDark_ST.z,_MainTexDark_ST.w);
dc_f_cb0[5]=float4(_Noise_ST.x,_Noise_ST.y,_Noise_ST.z,_Noise_ST.w);
dc_f_cb0[6]=float4(_USpeed,_VSpeed,_NoiseBrightness,_NoiseContrast);
dc_f_cb0[7]=float4(_Alpha,0,0,0);
  ...
  r1.xy = dc_f_cb0[6].xy * r0.ww;
  r1.xy = r1.xy * float2(0.00999999978,0.00999999978) + v1.xy;
  r1.xy = r1.xy * dc_f_cb0[5].xy + dc_f_cb0[5].zw;
  r1.xyzw = _Noise.Sample(sampler_Noise, r1.xy).xyzw;
  r0.w = dc_f_cb0[6].z + r1.x;
  r0.w = dc_f_cb0[6].w * r0.w;
  r1.xy = v1.xy * dc_f_cb0[4].xy + dc_f_cb0[4].zw;
  r1.xyzw = _MainTexDark.Sample(sampler_MainTexDark, r1.xy).xyzw;
