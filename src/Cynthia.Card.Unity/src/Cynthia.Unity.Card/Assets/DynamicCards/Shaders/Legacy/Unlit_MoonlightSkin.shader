Shader "DynamicCards/Legacy/Unlit_MoonlightSkin" {
Properties {
...
float4 dc_pack_OSGN_0=float4(0,0,0,0);
  ...
  dc_pack_OSGN_0.xy = v2.xy * dc_v_cb0[2].xy + dc_v_cb0[2].zw;
  dc_pack_OSGN_0.zw = v3.xy * dc_v_cb0[2].xy + dc_v_cb0[2].zw;
  o1.xyz = v1.xyz;
...
dc_f_cb0[3]=float4(_Progress,0,0,0);
...
float4 dc_pack_ISGN_0=float4(v0.x,v0.y,w0.x,w0.y);
  ...
  r0.x = 1 + -v1.x;
  r0.x = saturate(dc_f_cb0[3].x + r0.x);
  r1.xyzw = _MainTex.Sample(sampler_MainTex, dc_pack_ISGN_0.xy).xyzw;
  r2.xyzw = _MainTex.Sample(sampler_MainTex, dc_pack_ISGN_0.zw).xyzw;
