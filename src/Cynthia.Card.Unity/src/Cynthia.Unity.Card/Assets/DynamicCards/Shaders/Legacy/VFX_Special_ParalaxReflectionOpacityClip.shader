Shader "DynamicCards/Legacy/VFX_Special_ParalaxReflectionOpacityClip" {
Properties {
...
dc_f_cb0[5]=float4(_Displacedirection.x,_Displacedirection.y,_Displacedirection.z,_Displacedirection.w);
dc_f_cb0[6]=float4(_DisplaceNormal_ST.x,_DisplaceNormal_ST.y,_DisplaceNormal_ST.z,_DisplaceNormal_ST.w);
dc_f_cb0[7]=float4(_Unormaltiling,_Vnormaltiling,0,0);
dc_f_cb0[8]=float4(_Flowdirection.x,_Flowdirection.y,_Flowdirection.z,_Flowdirection.w);
dc_f_cb0[9]=float4(_Flowspeedmult,_Flowdisplaceamount,_UVnormalrotation,_Displaceamount);
dc_f_cb0[10]=float4(_AlphaExponent,_AlphaMultiplier,0,0);
  ...
  r0.x = dc_f_cb0[7].x * dc_f_cb0[7].y;
  r0.yz = -r0.xx * float2(0.5,0.5) + v1.xy;
  sincos(dc_f_cb0[9].z, r1.x, r2.x);
  r3.z = r1.x;
  r3.y = r2.x;
  r3.x = -r1.x;
  ...
  r0.xy = dc_f_cb0[7].xy * r0.xy;
  r0.z = dot(dc_f_cb0[8].xy, dc_f_cb0[8].xy);
  r0.z = rsqrt(r0.z);
  r0.zw = dc_f_cb0[8].xy * r0.zz;
  ...
  r0.x = exp2(r0.x);
  r0.x = saturate(dc_f_cb0[10].y * r0.x);
  r0.x = -0.5 + r0.x;
  r0.x = cmp(r0.x < 0);
  if (r0.x != 0) discard;
  o0.w = 1;
