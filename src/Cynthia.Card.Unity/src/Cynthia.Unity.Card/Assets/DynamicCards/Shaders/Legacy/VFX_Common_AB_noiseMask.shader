Shader "DynamicCards/Legacy/VFX_Common_AB_noiseMask" {
Properties {
...
dc_f_cb0[4]=float4(_OpacityContrast,_TiemSpeed,_Min,_Max);
dc_f_cb0[5]=float4(_OpacitySize,0,0,0);
dc_f_cb0[6]=float4(_Noise3_ST.x,_Noise3_ST.y,_Noise3_ST.z,_Noise3_ST.w);
dc_f_cb0[7]=float4(_NoiseSpeed2,_SubOpaCont,_SubOpaSize,0);
dc_f_cb0[8]=float4(_Noise01_ST.x,_Noise01_ST.y,_Noise01_ST.z,_Noise01_ST.w);
dc_f_cb0[9]=float4(_Sub_size,_DropSize,_DropNoiseSize,0);
dc_f_cb0[10]=float4(_TintColor2.x,_TintColor2.y,_TintColor2.z,_TintColor2.w);
dc_f_cb0[11]=float4(_Sharpness,_Shade,_MaskMovea,0);
dc_f_cb0[12]=float4(_UVNoise_ST.x,_UVNoise_ST.y,_UVNoise_ST.z,_UVNoise_ST.w);
dc_f_cb0[13]=float4(_SpecColour.x,_SpecColour.y,_SpecColour.z,_SpecColour.w);
  ...
  r0.w = dc_f_cb0[7].x * r0.z;
  r0.x = r0.w * 0.5 + v1.x;
  r0.xy = r0.xy * dc_f_cb0[12].xy + dc_f_cb0[12].zw;
  r1.xyzw = _UVNoise.Sample(sampler_UVNoise, r0.xy).xyzw;
  r2.z = r0.z * dc_f_cb0[7].x + v1.x;
  r2.x = r0.z * dc_f_cb0[4].y + v1.x;
  r2.yw = v1.yy;
  r0.xy = r2.zw * dc_f_cb0[12].xy + dc_f_cb0[12].zw;
  r0.xyzw = _UVNoise.Sample(sampler_UVNoise, r0.xy).xyzw;
  r0.x = r0.x + r1.x;
  r0.x = -1 + r0.x;
  r0.y = 1 + -v1.y;
  r0.y = saturate(v1.y * r0.y);
  r0.z = saturate(dc_f_cb0[7].y * r0.y + dc_f_cb0[7].z);
  r0.y = saturate(r0.y * dc_f_cb0[4].x + dc_f_cb0[5].x);
  r0.y = r0.y * 2 + -1;
  r0.z = 1 + -r0.z;
  r0.z = r0.z * 2 + -1;
  r0.w = 1.5 * dc_f_cb0[9].y;
  r1.xy = dc_f_cb0[9].xx * r2.xy;
  r1.xy = r1.xy * dc_f_cb0[8].xy + dc_f_cb0[8].zw;
  r1.xyzw = _Noise01.Sample(sampler_Noise01, r1.xy).xyzw;
  r0.w = r1.x / r0.w;
  r0.z = r0.z + -r0.w;
  r0.z = r0.x * dc_f_cb0[9].z + r0.z;
  r0.z = 1 + r0.z;
  r0.w = dc_f_cb0[11].z + v1.x;
  r0.w = log2(r0.w);
  r0.w = 20 * r0.w;
  r0.w = exp2(r0.w);
  r0.w = min(1, r0.w);
  r1.yz = r2.xy * dc_f_cb0[8].xy + dc_f_cb0[8].zw;
  r2.xy = r2.xy * dc_f_cb0[6].xy + dc_f_cb0[6].zw;
  r2.xyzw = _Noise3.Sample(sampler_Noise3, r2.xy).xyzw;
  r1.w = log2(r2.x);
  r1.xw = dc_f_cb0[11].yx * r1.xw;
  r1.w = exp2(r1.w);
  r1.w = min(1, r1.w);
  r2.xyzw = _Noise01.Sample(sampler_Noise01, r1.yz).xyzw;
  r0.y = -r2.x * 0.666666687 + r0.y;
  r0.x = r0.x * dc_f_cb0[9].z + r0.y;
  r0.x = 1 + r0.x;
  r0.xz = saturate(r0.wz * r0.xw);
  r0.x = r0.x + -r0.z;
  r0.y = r0.x * 2 + -1;
  r0.y = saturate(r0.y * r1.x);
  r1.xyz = -dc_f_cb0[10].xyz + dc_f_cb0[3].xyz;
  r1.xyz = r1.www * r1.xyz + dc_f_cb0[10].xyz;
  o0.xyz = r0.yyy * dc_f_cb0[13].xyz + r1.xyz;
  r0.y = dc_f_cb0[4].w + -dc_f_cb0[4].z;
  o0.w = r0.x * r0.y + dc_f_cb0[4].z;
  return;
}
ENDCG
}
