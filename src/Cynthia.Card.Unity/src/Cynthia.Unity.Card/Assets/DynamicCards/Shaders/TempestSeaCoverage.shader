Shader "DynamicCards/TempestSeaCoverage" {
Properties {
...
float2 CoverageUv(float2 uv) {
 if (uv.x < 0.68 && uv.y < 0.255)
   uv.y = 0.255 + 0.06 * abs(1.0 - 2.0 * frac((uv.y - 0.255) / 0.12 + 0.5));
 return uv;
}
...
float4 dc_pack_ISGN_1=float4(v1.x,v1.y,w1.x,w1.y);
  ...
  r0.xyzw = _FlowTex.Sample(sampler_FlowTex, CoverageUv(r0.xy)).xyzw;
  ...
  r1.xyzw = _MainTex.Sample(sampler_MainTex, CoverageUv(r0.zw)).xyzw;
  r0.xyzw = _MainTex.Sample(sampler_MainTex, CoverageUv(r0.xy)).xyzw;
