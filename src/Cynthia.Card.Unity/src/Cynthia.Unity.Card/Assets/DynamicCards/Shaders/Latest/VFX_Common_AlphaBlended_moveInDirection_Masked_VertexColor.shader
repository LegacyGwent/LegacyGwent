Shader "DynamicCards/Latest/VFX_Common_AlphaBlended_moveInDirection_Masked_VertexColor" {
Properties {
_Tint("_Tint",Color)=(1.0,1.0,1.0,1.0)
_UV_swap("_UV_swap",Float)=0.0
_U_speed("_U_speed",Float)=0.0
_V_speed("_V_speed",Float)=0.0
_Main_texture("_Main_texture",2D)="white" {}
_Mask_texture("_Mask_texture",2D)="white" {}
...
Texture2D<float4> _Main_texture;
Texture2D<float4> _Mask_texture;
...
dc_f_cb0[3]=float4(_UV_swap,_U_speed,_V_speed,0);
dc_f_cb0[4]=float4(_Main_texture_ST.x,_Main_texture_ST.y,_Main_texture_ST.z,_Main_texture_ST.w);
dc_f_cb0[5]=float4(_Mask_texture_ST.x,_Mask_texture_ST.y,_Mask_texture_ST.z,_Mask_texture_ST.w);
dc_f_cb0[6]=float4(_Use_vertex_color,_use_vertex_alpha,_Alpha_Exponent,_Alpha_Multiplier);
dc_f_cb0[7]=float4(_Mask_Color_Blend,_MainGain,0,0);
  ...
  r0.xy = v1.yx + -v1.xy;
  r0.xy = dc_f_cb0[3].xx * r0.xy + v1.xy;
  r0.xy = dc_f_cb0[3].yz * dc_f_cb1[0].xx + r0.xy;
  r0.xy = r0.xy * dc_f_cb0[4].xy + dc_f_cb0[4].zw;
  r0.xyzw = _Main_texture.Sample(sampler_Main_texture, r0.xy).xyzw;
  r0.xyz = dc_f_cb0[7].yyy * r0.xyz;
  r1.xy = v1.xy * dc_f_cb0[5].xy + dc_f_cb0[5].zw;
  r1.xyzw = _Mask_texture.Sample(sampler_Mask_texture, r1.xy).xyzw;
  r2.xyz = float3(1,1,1) + -r1.xyz;
  r2.xyz = dc_f_cb0[7].xxx * r2.xyz + r1.xyz;
  r1.x = r1.x * r1.w;
  r0.w = r1.x * r0.w;
  r0.xyz = r2.xyz * r0.xyz;
  r0.xyzw = dc_f_cb0[2].xyzw * r0.xyzw;
  r1.xyzw = float4(-1,-1,-1,-1) + v2.xyzw;
  r1.xyzw = dc_f_cb0[6].xxxy * r1.xyzw + float4(1,1,1,1);
  o0.xyz = r1.xyz * r0.xyz;
  r0.x = r1.w * r0.w;
  r0.x = log2(r0.x);
  r0.x = dc_f_cb0[6].z * r0.x;
  r0.x = exp2(r0.x);
  o0.w = saturate(dc_f_cb0[6].w * r0.x);
  return;
}
ENDCG
}
