...
#define cmp -
SamplerState sampler_BannerTex;
...
dc_v_cb0[4]=float4(_BannerTex_ST.x,_BannerTex_ST.y,_BannerTex_ST.z,_BannerTex_ST.w);
...
dc_v_cb0[14]=float4(_OffsetTex_ST.x,_OffsetTex_ST.y,_OffsetTex_ST.z,_OffsetTex_ST.w);
dc_v_cb0[15]=float4(_SpeedHUE,_OffsetMult,0,0);
...
float4 dc_pack_OSGN_1=float4(0,0,0,0);
  ...
  r1.zw = float2(1,-1);
  r2.xy = v1.xy * dc_v_cb0[14].xy + dc_v_cb0[14].zw;
  ...
  r0.y = r0.y / r1.y;
  r0.y = r0.z + r0.y;
  r0.y = dc_v_cb1[0].y * dc_v_cb0[15].x + abs(r0.y);
  ...
  r0.xyz = r0.xyz * dc_v_cb0[15].yyy + v0.xyz;
  ...
  o0.xyzw = r1.xyzw;
  dc_pack_OSGN_1.xy = v2.xy * dc_v_cb0[4].xy + dc_v_cb0[4].zw;
...
dc_f_cb0[2]=float4(_Saturation,_Brightness,0,0);
dc_f_cb0[3]=float4(_FrameTex_ST.x,_FrameTex_ST.y,_FrameTex_ST.z,_FrameTex_ST.w);
dc_f_cb0[4]=float4(0,0,0,0);
dc_f_cb0[5]=float4(0,0,0,0);
dc_f_cb0[6]=float4(_OverrideST.x,_OverrideST.y,_OverrideST.z,_OverrideST.w);
dc_f_cb0[7]=float4(_DissolveMask_ST.x,_DissolveMask_ST.y,_DissolveMask_ST.z,_DissolveMask_ST.w);
dc_f_cb0[8]=float4(_DissolveColorRamp_ST.x,_DissolveColorRamp_ST.y,_DissolveColorRamp_ST.z,_DissolveColorRamp_ST.w);
dc_f_cb0[9]=float4(_ColorGRadInfluence,_BlackBurnInfluence,_Progress,_EdgeMin);
dc_f_cb0[10]=float4(_EdgeMax,0,0,0);
dc_f_cb0[11]=float4(_SecondMask_ST.x,_SecondMask_ST.y,_SecondMask_ST.z,_SecondMask_ST.w);
dc_f_cb0[12]=float4(_SecondMaskMult,_SpeedU,_SpeedV,_PaperMax);
dc_f_cb0[13]=float4(_PaperMin,0,0,0);
dc_f_cb0[14]=float4(0,0,0,0);
dc_f_cb0[15]=float4(0,0,0,0);
dc_f_cb0[16]=float4(_TintColor.x,_TintColor.y,_TintColor.z,_TintColor.w);
dc_f_cb0[17]=float4(_BurnMaskInfl,0,0,0);
dc_f_cb0[18]=float4(_BurnEdge_ST.x,_BurnEdge_ST.y,_BurnEdge_ST.z,_BurnEdge_ST.w);
dc_f_cb0[19]=float4(_EdgeColor.x,_EdgeColor.y,_EdgeColor.z,_EdgeColor.w);
dc_f_cb0[20]=float4(_minRemap,_maxRemap,_DissolveProgress,0);
...
float4 dc_pack_ISGN_1=float4(v1.x,v1.y,w1.x,w1.y);
  ...
  r0.xy = dc_pack_ISGN_1.xy * dc_f_cb0[11].xy + dc_f_cb0[11].zw;
  r0.xyzw = _SecondMask.Sample(sampler_SecondMask, r0.xy).xyzw;
  r0.x = -1 + r0.x;
  r0.x = dc_f_cb0[12].x * r0.x + 1;
  r0.yz = v2.xy / v2.ww;
  r0.yz = dc_f_cb1[0].yy * dc_f_cb0[12].yz + r0.yz;
  r0.yz = r0.yz * dc_f_cb0[7].xy + dc_f_cb0[7].zw;
  r1.xyzw = _DissolveMask.Sample(sampler_DissolveMask, r0.yz).xyzw;
  r0.y = dc_f_cb0[20].z + dc_f_cb0[9].z;
  r0.z = 1 + -r0.y;
  r0.z = r0.z * 2 + r1.x;
  r0.z = -1 + r0.z;
  r0.x = r0.x * r0.z;
  r0.z = r0.x * dc_f_cb0[16].w + -0.5;
  r0.z = cmp(r0.z < 0);
  if (r0.z != 0) discard;
  r0.zw = dc_pack_ISGN_1.xy * dc_f_cb0[6].xy + dc_f_cb0[6].zw;
  r1.xyzw = _MainTex.Sample(sampler_MainTex, r0.zw).xyzw;
  r2.xyz = r1.xyz * float3(0.400000006,0.400000006,0.400000006) + float3(0.100000001,0.100000001,0.100000001);
  r1.xyz = -r2.xyz * float3(0.300000012,1,0.800000012) + r1.xyz;
  r2.xyz = float3(0.300000012,1,0.800000012) * r2.xyz;
  r1.xyz = dc_f_cb0[2].yyy * r1.xyz + r2.xyz;
  r0.z = dot(r1.xyz, float3(0.298839003,0.586811006,0.114349999));
  r1.xyz = r1.xyz + -r0.zzz;
  r1.xyz = dc_f_cb0[2].xxx * r1.xyz + r0.zzz;
  r0.zw = dc_pack_ISGN_1.xy * dc_f_cb0[3].xy + dc_f_cb0[3].zw;
  r2.xyzw = _FrameTex.Sample(sampler_FrameTex, r0.zw).xyzw;
  r2.xyz = r2.xyz + -r1.xyz;
  r1.xyz = r2.www * r2.xyz + r1.xyz;
  r2.xyzw = _BannerTex.Sample(sampler_BannerTex, dc_pack_ISGN_1.xy).xyzw;
  r2.xyz = r2.xyz + -r1.xyz;
  r1.xyz = r2.www * r2.xyz + r1.xyz;
  r0.z = dc_f_cb0[17].x * -r0.x + r1.x;
  r2.x = dc_f_cb0[18].x * r0.z;
  r2.yw = float2(0,0);
  r0.zw = dc_f_cb0[18].zw + r2.xy;
  r3.xyzw = _BurnEdge.Sample(sampler_BurnEdge, r0.zw).xyzw;
  r3.xyz = dc_f_cb0[19].xyz * r3.xyz;
  r0.z = dc_f_cb0[20].y + -dc_f_cb0[20].x;
  r3.xyz = r3.xyz * r0.zzz + dc_f_cb0[20].xxx;
  r3.xyz = -dc_f_cb0[16].xyz * r1.xyz + r3.xyz;
  r1.xyz = dc_f_cb0[16].xyz * r1.xyz;
  r1.xyz = r0.yyy * r3.xyz + r1.xyz;
  r1.xyz = float3(1,1,1) + -r1.xyz;
  r0.z = -dc_f_cb0[13].x + dc_f_cb0[12].w;
  r0.y = r0.y * r0.z + dc_f_cb0[13].x;
  r0.z = log2(r0.x);
  r0.y = r0.y * r0.z;
  r0.y = exp2(r0.y);
  r0.y = min(1, r0.y);
  ...
  r0.yzw = float3(1,1,1) + -r0.yzw;
  r1.x = dc_f_cb0[10].x + -dc_f_cb0[9].w;
  r0.x = r0.x * r1.x + dc_f_cb0[9].w;
  r1.x = saturate(r0.x);
  r0.x = saturate(-dc_f_cb0[9].y + r0.x);
  r1.x = 1 + -r1.x;
  r2.z = dc_f_cb0[8].x * r1.x;
  r1.xy = dc_f_cb0[8].zw + r2.zw;
