Shader "DynamicCards/Latest/VFX_Special_Unicorn_alchemist_fire" {
Properties {
...
dc_v_cb0[4]=float4(_Turbulence_ST.x,_Turbulence_ST.y,_Turbulence_ST.z,_Turbulence_ST.w);
dc_v_cb0[5]=float4(_Strength,0,0,0);
dc_v_cb0[6]=float4(_turbulence_speed,0,0,0);
...
float4 dc_pack_OSGN_1=float4(0,0,0,0);
  ...
  r0.y = dc_v_cb1[0].y * dc_v_cb0[6].x + v2.y;
  r0.x = v2.x;
  ...
  r0.xyz = dc_v_cb0[5].xxx * r0.xyz + v0.xyz;
...
dc_f_cb0[5]=float4(0,_text_speed,_texture_brightness,_texture_title_v);
...
float4 dc_pack_ISGN_1=float4(v1.x,v1.y,w1.x,w1.y);
  ...
  r0.x = dc_f_cb1[0].y * dc_f_cb0[5].y + dc_pack_ISGN_1.y;
  ...
  r1.xyz = dc_f_cb0[5].zzz * dc_f_cb0[3].xyz;
