Shader "DynamicCards/Latest/Custom_LensDirt_Liquid_AB_NoRef" {
Properties {
...
_InsideGradient("_InsideGradient",Float)=1.0
_powerColor("_powerColor",Float)=0.0
...
dc_f_cb0[3]=float4(_MainText_ST.x,_MainText_ST.y,_MainText_ST.z,_MainText_ST.w);
dc_f_cb0[4]=float4(_DistortStrength,0,0,0);
dc_f_cb0[5]=float4(_Mask_ST.x,_Mask_ST.y,_Mask_ST.z,_Mask_ST.w);
dc_f_cb0[6]=float4(_OpacityClip,_Brightnes,_Contrast,_VTEturetiling);
dc_f_cb0[7]=float4(_UTextureTiling,_InsideGradient,_powerColor,0);
  ...
  r0.x = exp2(r0.x);
  r0.yz = v2.xy * dc_f_cb0[3].xy + dc_f_cb0[3].zw;
  r1.xyzw = _MainText.Sample(sampler_MainText, r0.yz).xyzw;
  r0.y = r1.x * 2 + -1;
  r0.yz = r0.yy * dc_f_cb0[4].xx + v1.xy;
  r0.yz = r0.yz * dc_f_cb0[5].xy + dc_f_cb0[5].zw;
  r1.xyzw = _Mask.Sample(sampler_Mask, r0.yz).xyzw;
  r2.xyzw = dc_f_cb0[6].zzzx * r1.xyzx;
  r1.xyzw = r2.xyzw * r1.wwww;
  r0.yzw = saturate(dc_f_cb0[2].www * r1.xyz);
  r1.x = v3.w * r1.w;
  o0.w = dc_f_cb0[2].w * r1.x;
  r0.yzw = saturate(dc_f_cb0[6].yyy * v3.zzz + r0.yzw);
  r0.yzw = float3(1,1,1) + -r0.yzw;
  r0.xyz = dc_f_cb0[7].yyy * r0.xxx + r0.yzw;
  o0.xyz = dc_f_cb0[2].xyz * r0.xyz;
  return;
}
ENDCG
}
