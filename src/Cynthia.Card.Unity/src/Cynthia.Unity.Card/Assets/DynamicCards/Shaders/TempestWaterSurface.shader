Shader "DynamicCards/TempestWaterSurface" {
Properties {
[Toggle] _LayerEnabled("Foreground water visible",Float)=1.0
...
float4 dc_pack_ISGN_1=float4(v1.x,v1.y,w1.x,w1.y);
  ...
  // The atlas alpha defines a fixed surface silhouette. Advecting that mask
  // through an alpha-test threshold chops pieces off the crest every cycle.
  float4 sourceSample = _MainTex.Sample(sampler_MainTex, dc_pack_ISGN_1.xy);
  float sourceAlpha = sourceSample.a * v6.a;
  float edgeFlow = smoothstep(0.5, 0.95, sourceAlpha);
  ...
  r0.xyzw = (dc_f_cb0[18].yyyy * edgeFlow) * r0.xyzw;
  ...
  // Flow coordinates near the atlas silhouette can leave valid water pixels.
  // Fade to the original surface there instead of exposing padded atlas RGB.
  r0.xyzw = lerp(sourceSample, r0.xyzw, smoothstep(0.6, 0.98, r0.a));
  r0.xyzw = dc_f_cb0[7].xyzw * r0.xyzw;
  r0.xyzw = v6.xyzw * r0.xyzw;
  r0.w = sourceAlpha * _TintColor.a;
  r1.x = 1 + -dc_f_cb0[3].z;
