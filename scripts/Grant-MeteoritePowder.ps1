...
if ([string]::IsNullOrWhiteSpace($MongoShell)) { throw 'Mongo shell was not found. Install mongosh/mongo or set MONGO_SHELL to an executable path.' }
if (!(Test-Path -LiteralPath $MongoShell -PathType Leaf)) { throw "Mongo shell was not found: $MongoShell. Set MONGO_SHELL to an executable path." }
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { Write-Error 'Powder grant failed.' -ErrorAction Continue; exit $LASTEXITCODE }
