...
- 上游合并候选的 .NET Release solution、运行镜像和启动健康检查通过 Linux CI；
  服务器兼容测试 68 项、玩法回归 296 项通过。
- 客户端契约检查 30 项通过，包括普通包不擦除闪卡选择和不覆盖画质偏好。
- 隔离 MongoDB 上的合成、每日、GG、同对手奖励检查共 159 项通过；完整奖励回归
  含故障注入共 281 项通过。合并时修复了任意旧牌组 ID 导致保存队列阻塞、旧协议
